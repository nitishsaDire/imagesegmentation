...
def train_model(unet, optimizer, scheduler, dataloader, dataset_sizes, device, loadModel = False, num_epochs=100):
    ...
    OLD_PATH = '/content/drive/MyDrive/sem_is_dice-bce100'
    PATH = '/content/drive/MyDrive/sem_is_dice-bce2'
    epoch = 0
    if loadModel == True:
            ...
            for phase in ['train', 'val']:
                    ...
                    for inputs, mask_1c in dataloader[phase]:
                            ...
                            with torch.set_grad_enabled(phase == 'train'):
                                    ...
                                    print("tsm", 0.7 * F.binary_cross_entropy_with_logits(outputs.to(device), mask.to(torch.float)),\
                                             0.3 * dice_loss(torch.sigmoid(outputs.to(device)), mask.to(torch.float)))
                            ...
                            running_corrects += (1 - dice_loss(torch.sigmoid(outputs.to(device)), mask.to(torch.float))) * inputs.size(0)
